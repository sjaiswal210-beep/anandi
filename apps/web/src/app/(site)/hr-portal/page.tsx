...
export default function StandaloneHrPortal() {
  ...
  useEffect(() => {
    const checkAuthAndLoad = async () => {
      ...
      if (savedUnlock === 'yes') {
        try {
          // Perform silent background login to restore valid access token on this tab
          ...
          console.error('Silent login restore failed:', err);
          setUnlocked(true); // Fallback to unlock but fetch might trigger auth checks
        }
      }
  ...
  // 2. Fetch data upon successful unlock
  useEffect(() => {
    if (unlocked) {
      fetchEmployees();
      fetchLogs();
  ...
  // 3. Fetch payroll on month/year changes
  useEffect(() => {
    if (unlocked && activeTab === 'payroll') {
      fetchPayroll();
    }
  }, [unlocked, activeTab, payrollMonth, payrollYear]);
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    if (password === 'richland2026') {
      ...
      setError('');
      try {
        // Authenticate background API gateway session
        const res: any = await api.post('/auth/login', { email: 'Kalpdev@outlook.com', password: 'Kalpdev@1234' });
        const token = res?.data?.accessToken || res?.accessToken;
        if (token) {
          localStorage.setItem('realtyos-auth', JSON.stringify({ state: { token } }));
  ...
  const handleCopyLink = (url: string, label: string) => {
    ...
    alert(`${label} link copied to clipboard! Send to team members via WhatsApp.`);
  };
  ...
  const handleApproveLeave = async (id: string, status: 'APPROVED' | 'REJECTED') => {
    setActionLoading(true);
    try {
      await api.put(`/hr/leaves/${id}/approve`, { status, userId: 'clvk...' });
      fetchLeaves();
      fetchLogs();
    } catch (err: any) {
      try {
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-sm font-semibold text-muted-foreground">Authenticating HR Workspace...</p>
        </div>
      </div>
    );
  }
  ...
  if (!unlocked) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6">
        <div className="max-w-md w-full bg-card border rounded-2xl p-8 shadow-xl space-y-6 text-center">
          <div className="mx-auto w-14 h-14 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center">
            <Lock className="h-7 w-7" />
          </div>
          
          <div className="space-y-1">
            <h1 className="text-2xl font-black tracking-tight text-foreground">Rich-Land Developers</h1>
            <p className="text-sm text-muted-foreground font-semibold">Standalone Attendance & HR Portal</p>
          </div>

          <form onSubmit={handleUnlock} className="space-y-4">
            <div className="space-y-1.5 text-left">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Secure Portal Password</label>
              <input 
                type="password"
                required
                value={password}
                ...
                placeholder="••••••••"
                className="w-full border rounded-xl p-3 bg-background text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none font-sans text-center text-lg tracking-widest"
              />
            </div>

            {error && (
              <div className="flex items-center gap-2 p-3 text-rose-600 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 rounded-lg text-xs font-semibold text-left">
          ...
          <p className="text-xs text-muted-foreground font-sans">
            Secure, role-restricted access. This portal is configured exclusively for authorized managers.
          </p>
        </div>
      </div>
    );
  ...
  // Full HR Portal Screen (Without sidebar)
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950/50">
      {/* Top Banner Navigation bar (Replaces Sidebar layout context) */}
      <header className="bg-card border-b sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-emerald-600 text-white rounded-lg flex items-center justify-center font-bold text-base">R</div>
            <div>
              <span className="font-extrabold text-base tracking-tight text-foreground block leading-none">Rich-Land Developers</span>
              <span className="text-[10px] text-emerald-600 font-bold tracking-widest uppercase">HR Portal</span>
            </div>
          </div>

          <button 
            onClick={handleLogout}
            className="flex items-center gap-1.5 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/40 hover:bg-rose-100 text-rose-700 dark:text-rose-400 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
          >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="bg-slate-100/60 dark:bg-slate-900/30 border rounded-2xl p-5 grid grid-cols-1 md:grid-cols-4 gap-5">
